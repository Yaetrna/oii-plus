...
function formatGameMode(mode) {
  const modes = { osu: 'osu!standard', taiko: 'osu!taiko', fruits: 'osu!catch', mania: 'osu!mania' };
...
async function sendToContentScript(message) {
  try {
    if (!browserAPI) return { success: false, error: 'Browser API not available' };
    const [tab] = await browserAPI.tabs.query({ active: true, currentWindow: true });
    if (!tab?.url?.includes('osu.ppy.sh/users/')) return { success: false, error: 'Not on an osu! profile page' };
...
async function handleAdditionalPlaytimeChange(event) {
  ...
  if (additionalHours < 0) { event.target.value = 0; return; }
  if (additionalHours === 0) { hideAdjustedII(); return; }

  const response = await sendToContentScript({ type: 'UPDATE_PLAYTIME', additionalPlaytimeHours: additionalHours });
  if (response.success) {
    ...
    if (dataResponse.success && dataResponse.data) {
      ...
      if (totalHits > 0) {
        ...
        showAdjustedII(`Adjusted II: ${adjustedII.toFixed(2)}x (with +${additionalHours}h)`, 'highlight');
      }
    }
  }
}
...
async function handleGoalPPChange(event) {
  ...
  if (goalPP <= 0) { hidePrediction(); return; }

  const response = await sendToContentScript({ type: 'GET_PREDICTION', goalPP, additionalPlaytimeHours: additionalHours });
  if (response.success) {
    ...
    if (additionalNeeded <= 0) {
      showPrediction(`🎉 You should already be at ${goalPP}pp based on your improvement rate!`, 'success');
    } else {
      showPrediction(`📊 Estimated time to reach ${goalPP.toLocaleString()}pp: ${formatPlaytime(remaining)} total (${formatPlaytime(additionalNeeded)} more)`, 'highlight');
    }
  } else {
...
async function init() {
  const profileLinks = [
    'https://osu.ppy.sh/users/32693046', 'https://osu.ppy.sh/users/35533437',
    'https://osu.ppy.sh/users/13234370', 'https://osu.ppy.sh/users/27514055',
    'https://osu.ppy.sh/users/22094038', 'https://osu.ppy.sh/users/14293652',
    'https://osu.ppy.sh/users/13910074', 'https://osu.ppy.sh/users/7388142',
    'https://osu.ppy.sh/users/20726353', 'https://osu.ppy.sh/users/16363979',
    'https://osu.ppy.sh/users/30556250', 'https://osu.ppy.sh/users/11862201',
    'https://osu.ppy.sh/users/11705938', 'https://osu.ppy.sh/users/14661718',
    'https://osu.ppy.sh/users/15840146', 'https://osu.ppy.sh/users/16523449',
    'https://osu.ppy.sh/users/13223964', 'https://osu.ppy.sh/users/14457718'
  ];
  
  const randomLink = document.getElementById('randomProfileLink');
  if (randomLink) randomLink.href = profileLinks[Math.floor(Math.random() * profileLinks.length)];
